...
class ToggleNotificationCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $toggleTo = $input->getArgument('toggleTo') ? \DateTime::createFromFormat(
            'Y-m-d H:i:s',
            $input->getArgument('toggleTo')
        ) : null;
        ...
        if ($toggleTo) {
            $output->writeln(
                sprintf(
                    'Notification has been <info>%s</info> until <info>%s</info>.',
                    ($enable ? 'enabled' : 'disabled'),
                    $toggleTo->format('Y-m-d H:i:s')
                )
            );
        } else {
